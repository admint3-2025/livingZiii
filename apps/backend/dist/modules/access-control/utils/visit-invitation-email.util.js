...
exports.resolveQrInlineImage = resolveQrInlineImage;
exports.buildVisitInvitationEmailHtml = buildVisitInvitationEmailHtml;
exports.buildVisitInvitationEmailText = buildVisitInvitationEmailText;
exports.extractInlinePng = extractInlinePng;
...
function normalizeQrRawValue(qrCode) {
    return (qrCode || '').trim();
}
...
function extractInlineImage(qrCode) {
...
async function resolveQrInlineImage(qrCode) {
    ...
    try {
        const buffer = await qrcode_1.default.toBuffer(rawValue, {
            type: 'png',
            width: 1200,
            margin: 2,
...
function describeUsage(maxEntries) {
...
function buildVisitInvitationEmailHtml({ invitation, shareUrl, logoCid, qrCid, unitLabel, residentName, }) {
    ...
    const allowedDoors = Array.isArray(invitation.metadata?.allowedDoors) && invitation.metadata.allowedDoors.length
        ? invitation.metadata.allowedDoors.map((door) => escapeHtml(door)).join(', ')
        : 'Acceso autorizado';
    const usageLabel = describeUsage(Number(invitation.metadata?.maxEntries ?? 1));
    const showRawQrFallback = Boolean(invitation.qrCode && invitation.qrCode.length <= 120 && !invitation.qrCode.startsWith('data:image'));
...
function buildVisitInvitationEmailText(invitation, shareUrl, residentName) {
    const allowedDoors = Array.isArray(invitation.metadata?.allowedDoors) && invitation.metadata.allowedDoors.length
...
function extractInlinePng(qrCode) {
    const inlineImage = extractInlineImage(qrCode);
...
//# sourceMappingURL=visit-invitation-email.util.js.map